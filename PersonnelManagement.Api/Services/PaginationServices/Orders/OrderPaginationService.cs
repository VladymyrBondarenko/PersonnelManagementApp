...
namespace PersonnelManagement.Server.Services.PaginationServices.Orders
{
    public class OrderPaginationService : IOrderPaginationService
    {
        private readonly IUriService _uriService;

        public OrderPaginationService(IUriService uriService)
        {
        ...
        public PagedResponse<T> CreatePaginatedResponse<T>(PaginationQuery paginationQuery, List<T> response, int totalAmount)
        {
            var nextPage = paginationQuery.PageNumber >= 1
                ? _uriService.GetAllOrdersUri(new PaginationQuery(paginationQuery.PageNumber + 1, paginationQuery.PageSize)).ToString()
                : null;
