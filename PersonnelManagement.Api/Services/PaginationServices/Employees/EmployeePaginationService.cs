...
namespace PersonnelManagement.Server.Services.PaginationServices.Employees
{
    public class EmployeePaginationService : IEmployeePaginationService
    {
        private readonly IUriService _uriService;

        public EmployeePaginationService(IUriService uriService)
        {
        ...
        public PagedResponse<T> CreatePaginatedResponse<T>(PaginationQuery paginationQuery, List<T> response, int totalAmount)
        {
            var nextPage = paginationQuery.PageNumber >= 1
                ? _uriService.GetAllEmployeesUri(new PaginationQuery(paginationQuery.PageNumber + 1, paginationQuery.PageSize)).ToString()
                : null;
